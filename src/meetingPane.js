...
export default {
  ...
  render: function (subject, dataBrowserContext) {
    ...
    */
    ...
    iconCell.classList.add('meetingToolBarIcons')
    const iconRow = iconCell.appendChild(dom.createElement('div'))
    iconRow.classList.add('meetingToolIconRow')
    const parameterCell = toolBar.appendChild(dom.createElement('td'))
    const star = iconRow.appendChild(dom.createElement('img'))
    star.classList.add(
      'meetingToolIcon',
      'meetingToolIcon-visible',
      'meetingToolIconAddButton'
    )
    ...
    // I have these classes meetingToolIcon, meetingToolIcon-visible, meetingToolIconAddButton,
    // meetingToolIcon-disabled, meetingToolIcon-yellow.  I want to toggle the visibility of
    // the icons by adding or removing the class meetingToolIcon-visible.  Write a function
    // that takes a boolean showIcons and toggles the class on all the icons in iconArray accordingly.
    const toggleIconsVisibility = function (showIcons) {
      for (let i = 0; i < iconArray.length; i++) {
        iconArray[i].classList.toggle('meetingToolIcon-visible', showIcons)
      }
    }

    const selectNewTool = function (_event) {
      visible = !visible
      star.classList.toggle('meetingToolIcon-highlighted', visible)
      toggleIconsVisibility(visible)
    }
    ...
    for (let i = 0; i < toolIcons.length; i++) {
      const foo = function () {
        ...
        const icon = iconRow.appendChild(dom.createElement('img'))
        icon.classList.add('meetingToolIcon')
        icon.setAttribute('src', UI.icons.iconBase + toolObject.icon)
        if (toolObject.disabled) {
          icon.classList.add('meetingToolIcon-disabled')
        }
        iconArray.push(icon)
    ...
    const resetTools = function () {
      ...
      star.classList.remove('meetingToolIcon-highlighted')
    }
    ...
    const selectTool = function (icon) {
      ...
      toggleIconsVisibility(false)
      for (let i = 0; i < iconArray.length; i++) {
        iconArray[i].classList.remove('meetingToolIcon-highlighted')
      }
      icon.classList.add('meetingToolIcon-visible', 'meetingToolIcon-highlighted')
      star.classList.remove('meetingToolIcon-highlighted')
    }
    ...
    const renderTab = function (div, item) {
      if (kb.holds(item, ns.rdf('type'), ns.meeting('Tool'))) {
        ...
        s.classList.add('meetingToolLabel')
        s.textContent = label
        ...
        if (icon) {
          ...
          img.classList.add('meetingToolIconImg')
    ...
    const tipDiv = function (text) {
      ...
      p.classList.add('meetingToolTip')
    ...
    const renderTabSettings = function (containerDiv, subject) {
      ...
      if (kb.holds(subject, ns.rdf('type'), ns.meeting('Tool'))) {
        ...
        delButton.classList.add('meetingPaneDeleteButton')
      } else {
...
*/
        iframe.classList.add('meetingPaneIframe')
      ...
      const renderDetails = function () {
        ...
        spawn.classList.add('meetingToolIconSpawn')
        ...
        fork.classList.add('meetingToolIconForkGithub')
      ...
      if (kb.holds(subject, ns.rdf('type'), ns.meeting('Tool'))) {
        ...
        if (target.sameTerm(meeting) && !kb.any(subject, ns.meeting('view'))) {
          ...
          if (view === 'details') {
            ...
            table.classList.add('meetingPaneTableMain')
            dataBrowserContext
              .getOutliner(dom)
